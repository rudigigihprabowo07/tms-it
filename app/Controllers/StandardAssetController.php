...
class StandardAssetController extends ResourceController
{
    protected $modelName = 'App\Models\StandardAssetModel';
    ...
    {
        $data = $this->model->getStandardAsset();
        if ($data != null) {
    ...
    {
        $data = $this->model->getSpecificStandardAsset($id);
        if ($data) {
            return $this->respond($data, 200);
        } else {
            return $this->failNotFound();
